...
public class DatabaseConnection {
    ...
    public func open() throws {
        try SQLiteResultHandler.verifyResultCode(resultCode: sqlite3_open(path, &handle), forHandle: handle!)
        isOpen = true
    }
    
    /** Close the database connection */
    public func close() throws {
        try SQLiteResultHandler.verifyResultCode(resultCode: sqlite3_close(handle), forHandle: handle!)
        handle = nil
        isOpen = false
    }
    ...
    public func prepare(query: String) throws -> Statement {
        ...
        try statement.prepareForDatabase(databaseHandle: handle!)
        return statement
    }
}
